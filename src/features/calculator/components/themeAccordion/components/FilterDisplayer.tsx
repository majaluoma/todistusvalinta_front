...
export default function FilterDisplayer({
  ...
  const renderFilterButton = (key: keyof Filters, label: string) => (
    <Button className="px-0" variant="ghost" size="sm" onClick={() => handleClick(key)}>
      <span className="bg-transparent text-primary px-3 py-1 rounded-md hover:bg-primary hover:text-secondary">
        {label}
        <span className={`text-secondary ml-1 text-sm font-bold`}>x</span>
      </span>
    </Button>
  );
  ...
  return (
    <div className="flex flex-wrap">
      ...
      {filters.vocationalUnviersities !==
        defaultFilters.vocationalUnviersities &&
        renderFilterButton('vocationalUnviersities', 'Ei AMK-kohteita')}
      {filters.isSpring !==
        defaultFilters.isSpring &&
        renderFilterButton('isSpring', 'Syksyn yhteishaku')}
    </div>
  );
}
